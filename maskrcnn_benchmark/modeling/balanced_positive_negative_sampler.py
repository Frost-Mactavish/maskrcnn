...
class BalancedPositiveNegativeSampler(object):
    ...
    def __call__(self, matched_idxs, objectness=None):
        """
        Arguments:
        ...
        for matched_idxs_per_image in matched_idxs:
            ...
            #        highest_obj_idx = torch.argsort(objectness_neg, descending=True)
            #        #neg_idx_per_image = torch.hstack((negative[perm_neg[0:int(perm_neg.numel() / 2)]], negative[highest_obj_idx[0:int(perm_neg.numel()/2)]]))
            #        neg_idx_per_image = negative[highest_obj_idx][0:num_neg]
