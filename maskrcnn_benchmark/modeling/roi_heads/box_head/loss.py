...
class FastRCNNLossComputation(object):
    ...
    def subsample(self, proposals, targets, attention_maps=None):
        ...
        """
        if self.unk_enable:
            labels, regression_targets, weights, unk_idxes, unk_proposals = self.prepare_targets(proposals, targets,
    ...
    def __call__(self, class_logits, box_regression):
        ...
        if self.uce:
            ...
            outputs[:, self.n_old_cl + 1:] = class_logits[:, self.n_old_cl + 1:] - den.unsqueeze(
                dim=1)  # B, N, H, W    p(N_i)
