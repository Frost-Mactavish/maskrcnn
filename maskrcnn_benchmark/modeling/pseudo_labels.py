    ...
    for per_pse, per_gt in zip(pseudo_targets, gt_targets):
        ...
        if high_iouvalid_row_indices.numel() == 0 and low_iouvalid_row_indices.numel() == 0:
            ...
            if high_iouvalid_row_indices.numel() > 0:
                ...
                high_iou_pse_target_weight = torch.full_like(per_high_iou_pseudo_targe_label, high_weight,
                                                             dtype=torch.float)
            ...
            if low_iouvalid_row_indices.numel() > 0:
                ...
                low_iou_pse_target_weight = torch.full_like(per_low_iou_pseudo_targe_label, low_weight,
                                                            dtype=torch.float)
