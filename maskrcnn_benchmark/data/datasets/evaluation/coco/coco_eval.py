...
def convert_category_id_from_to_alphabetical_sequence_to_coco_sequence(index):
    new_index = -1
    cats_to_ids = dict(map(reversed, enumerate(COCO_CATS)))
    ids_coco_sequence_to_alphabetical_sequence = {k: cats_to_ids[v] for k, v in coco_ids_to_cats.items()}
    ids_alphabetical_sequence_to_coco_sequence = dict(map(reversed, ids_coco_sequence_to_alphabetical_sequence.items()))
    for key in ids_alphabetical_sequence_to_coco_sequence:
        if key == index:
            new_index = ids_alphabetical_sequence_to_coco_sequence[key]
            break
    if new_index == -1:
...
def do_coco_evaluation(
    ...
    if "bbox" in iou_types:
        ...
        coco_results["bbox"] = prepare_for_coco_detection(predictions, dataset, alphabetical_order)
    ...
    for iou_type in iou_types:
        with tempfile.NamedTemporaryFile() as f:
            ...
            res = evaluate_predictions_on_coco(dataset, coco_results[iou_type], file_path, iou_type)
            results.update(res)
    logger.info(results)
...
def prepare_for_coco_detection(predictions, dataset, alphabetical_order=False):
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        mapped_labels = []
        for i in labels:
            # print('current label: {0}'.format(i))
            if alphabetical_order:
                new_label = convert_category_id_from_to_alphabetical_sequence_to_coco_sequence(i)
                mapped_labels.append(new_label)
                # print('new_label: {0}'.format(new_label))
            else:
                # convert type continuous labels to COCO labels
                mapped_labels.append(dataset.contiguous_category_id_to_json_id[i])
...
def evaluate_predictions_on_coco(dataset, coco_results, json_result_file, iou_type="bbox"):
    import json
    ...
    category_ids = dataset.get_included_cats()

    # set parameters for COCO evaluation
...
if __name__ == '__main__':
    ...
    ids_coco_sequence_to_alphabetical_sequence = {k: cats_to_ids[v] for k, v in coco_ids_to_cats.items()}
    ids_alphabetical_sequence_to_coco_sequence = dict(map(reversed, ids_coco_sequence_to_alphabetical_sequence.items()))
    print('ids_coco_sequence_to_alphabetical_sequence: {0}'.format(ids_coco_sequence_to_alphabetical_sequence))
    print('ids_alphabetical_sequence_to_coco_sequence: {0}'.format(ids_alphabetical_sequence_to_coco_sequence))
