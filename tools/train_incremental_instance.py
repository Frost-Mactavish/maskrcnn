...
def do_train(model_source, model_target, data_loader, optimizer, scheduler, checkpointer_source, checkpointer_target,
    ...
    for iteration, (images, targets, _, idx) in tqdm(enumerate(data_loader, start_iter), total=len(data_loader)):
        ...
        with torch.no_grad():
            ...
            _, _, target_mask_logits, _ = model_target.roi_heads.calculate_soften_label(features, soften_proposal,
                                                                                        soften_result)
        ...
        target_result, roi_align_features_target = model_target.forward(images, targets, features=feature_target,
                                                                        proposals=soften_proposal)

        if cfg.DIST.CLS > 0:
            distillation_losses = calculate_roi_distillation_losses(soften_result, soften_mask_logits,
                                                                    target_mask_logits, target_result, dist=dist_type)
        else:
