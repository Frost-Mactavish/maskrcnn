...
class Mem(object):
    def __init__(self, cfg, step=0, current_mem_path=None):
        ...
        if self.STEP == 0:
            ...
            self.current_mem_path = os.path.join(f"log/{self.cfg.DATASET}/{self.cfg.TASK}/{self.cfg.NAME}",
                                                 self.current_mem_name)
            self.exemplar = os.listdir(self.current_mem_path)
    ...
    def rnd_sampling(self):
        ...
        for i in range(self.num_current_classes):
            ...
            if len(self.current_mem_info[i]) < self.num_bbox_per_cls:
                self.current_mem_info[i].extend(
                    self.current_mem_info[i][:self.num_bbox_per_cls - len(self.current_mem_info[i])])
        ...
        assert len(os.listdir(
            self.current_mem_path)) >= self.mem_size, 'The selected rehearsals are not satisfied the setting size!'
