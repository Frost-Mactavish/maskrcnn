...
def main():
    ...
    parser.add_argument("-t", "--task", type=str, default="10-10")
    parser.add_argument("-s", "--step", default=0, type=int)
    parser.add_argument("-mb", "--memory_buffer", default=2000, type=int)
    parser.add_argument("-mt", "--memory_type", default="mean", type=str,
                        choices=['random', 'herding', 'mean'])
    parser.add_argument("-iss", "--is_sample", default=True, action='store_true')
    ...
    if args.step == 0:
        cfg_source.MODEL.SOURCE_WEIGHT = f"{full_name}/STEP{args.step}/model_trimmed.pth"
        cfg_source.MODEL.WEIGHT = cfg_source.MODEL.SOURCE_WEIGHT
    elif args.step >= 1:
        cfg_source.MODEL.WEIGHT = f"{full_name}/STEP{args.step}/model_trimmed.pth"
        cfg_source.OUTPUT_DIR = full_name
